...
def get_data_for_a_query(query_dict, dataset_id):
    ...
    try:
        ...
        logger.info(
            f"Data for product {query_dict['PRODUCTCODE']} and partner {query_dict['PARTNER']} appended successfully"
        )
...
def ingest_all_unctad(minio_client, reporter_name, dataset_id, datatype_id):
    ...
    reporter_code = country_list_data[country_list_data["name"] == reporter_name][
        "id"
    ].tolist()[0]
        ...
        for product_id in product_list:
            ...
            if data is not None:
                total_data = (
                    data
                    if total_data.empty
                    else pd.concat([total_data, data], ignore_index=True)
                )
        upload_parquet(
            minio_client,
            os.getenv("MINIO_BUCKET_NAME"),
            total_data,
            f"unctad/{reporter_name.replace(' ','_')}_to_{partner_code}_unctad_data.parquet",
        )
