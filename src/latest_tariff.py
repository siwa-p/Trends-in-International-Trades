...
from utils.utilities import get_csv_from_lake, get_minio_client, upload_parquet
...
def process_chapter99(tariff_2025_filtered):
    chapter99 = tariff_2025_filtered[
        tariff_2025_filtered["hts8"].astype(str).str.startswith("99")
    ].copy()
    chapter99["extra_rate_text"] = chapter99.apply(
        lambda row: (
            extract_extra_rate(row["mfn_text_rate"])
            if row["mfn_ad_val_rate"] >= 9999
            else 0
        ),
        axis=1,
    )
...
def merge_tariff_data(tariff_2025_filtered, china_tariff, chapter99):
    ...
    merged = merged.merge(
        chapter99[
            ["hts8", "surcharge_rate", "begin_effect_date", "end_effective_date"]
        ],
        left_on="mapped_99_code",
        right_on="hts8",
        how="left",
        suffixes=("", "_99"),
    )
    merged["effective_ad_val_rate"] = merged["mfn_ad_val_rate"] + merged[
        "surcharge_rate"
    ].fillna(0)
    return merged
...
def aggregate_tariff_data(merged):
    ...
    merged_grouped["end_effective_date"] = merged_grouped["end_effective_date"].clip(
        upper=cutoff_date
    )
    merged_grouped["end_effective_date_99"] = merged_grouped[
        "end_effective_date_99"
    ].clip(upper=cutoff_date)
    ...
    for _, row in merged_copy.iterrows():
        for month in all_months:
            if row["begin_effect_date"] <= month <= row["end_effective_date"]:
                ...
                if (
                    pd.notnull(row["begin_effect_date_99"])
                    and row["begin_effect_date_99"]
                    <= month
                    <= row["end_effective_date_99"]
                ):
                    surcharge_rate = (
                        row["surcharge_rate"]
                        if pd.notnull(row["surcharge_rate"])
                        else 0
                    )
                ...
                timeline_rows.append(
                    {
                        ...
                        "surcharge_rate": (
                            surcharge_rate if "surcharge_rate" in locals() else 0
                        ),
...
def main():
    ...
    tariff_2025 = get_csv_from_lake(
        minio_client, os.getenv("MINIO_BUCKET_NAME"), "tariff_database_2025.csv"
    )
    logger.info(f"Loaded tariff_databse from Minio")
    china_tariff = get_csv_from_lake(
        minio_client, os.getenv("MINIO_BUCKET_NAME"), "china_tarriffs.csv"
    )
    ...
    tariff_2025_filtered_cleaned = tariff_2025_filtered[
        tariff_2025_filtered["mfn_other_rate"] != 9999.999999
    ]
