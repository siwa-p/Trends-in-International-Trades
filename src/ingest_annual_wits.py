...
def get_trade_and_tariff_aggregates(reporter:str, year):
    ...
    tariff_indicators = indicator_list[indicator_list['name'].str.contains('%') &  indicator_list['name'].str.contains('Tariff')]['id'].tolist()
    logger.info(f"Tariff Indicators: {tariff_indicators}")
    trade_indicators = indicator_list[indicator_list['name'].str.contains('Trade')]['id'].tolist()
    ...
    for product in product_list:
        ...
        for indicator in trade_indicators:
            ...
            if data is not None:
                trade_data = data if trade_data.empty else pd.concat([trade_data, data], ignore_index=True)     
        trade_data_all_products = trade_data if trade_data_all_products.empty else pd.concat([trade_data_all_products, trade_data], ignore_index=True)
        ...
        for indicator in tariff_indicators:
            ...
            if data is not None:
                tariff_data = data if tariff_data.empty else pd.concat([tariff_data, data], ignore_index=True)
            
        tariff_data_all_products = tariff_data if tariff_data_all_products.empty else pd.concat([tariff_data_all_products, tariff_data], ignore_index=True)
        
    logger.info(f"Trade data for {reporter} for year {year} fetched successfully with {len(trade_data_all_products)} records")
    logger.info(f"Tariff data for {reporter} for year {year} fetched successfully with {len(tariff_data_all_products)} records")
...
def ingest_all_data(reporter:str, startyear=2017, endyear=2025):
    ...
    for year in range(startyear, endyear+1):
        ...
        trade_data_total = trade_data if year == startyear else pd.concat([trade_data_total, trade_data], ignore_index=True)
        tariff_data_total = tariff_data if year == startyear else pd.concat([tariff_data_total, tariff_data], ignore_index=True)
        logger.info(f"Data for year {year} fetched successfully")
    
    upload_parquet(minio_client, os.getenv("MINIO_BUCKET_NAME"), trade_data_total, 'wits_trade_data.parquet')
    upload_parquet(minio_client, os.getenv("MINIO_BUCKET_NAME"), tariff_data_total, 'wits_tariff_data.parquet')
