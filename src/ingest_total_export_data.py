...
def get_monthly_port_data(year, month, trade_type, data_type):
    ...
    try:
        ...
        for commodity in ["1*", "2*", "3*", "4*", "5*", "6*", "7*", "8*", "9*"]:
            ...
            logger.info(
                f"Data fetched successfully for commodity {commodity} ({trade_type})"
            )
            monthly_dataframe = (
                pd.concat([monthly_dataframe, commodity_dataframe], ignore_index=True)
                if not monthly_dataframe.empty
                else commodity_dataframe
            )
        logger.info(
            f"Data for {year}-{str(month).zfill(2)} ({trade_type}) fetched successfully with {len(monthly_dataframe)} records"
        )
        ...
        for year in range(2015, 2026):
            ...
            for month in range(1, 13):
                ...
                if monthly_data is not None:
                    annual_data = (
                        pd.concat([annual_data, monthly_data], ignore_index=True)
            ...
            upload_parquet(
                ...
                annual_data,
                f"{trade_type}_{data_type}/total_{trade_type}_{data_type}{year}.parquet",
            )
    return None
